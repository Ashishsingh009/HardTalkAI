...
- **`server/`** — **FastAPI** (Python) backend on `:3001`. Serves the coaching engine
  ...
  `/api/scenarios`, `/api/chat`, `/api/voice/session`, `/api/voice/complete`.
  - **AI support:** when `OPENAI_API_KEY` is set, the counterpart's typed reply is generated by an
    LLM in character (`OPENAI_MODEL` overrides the model) and Android can start a live
    counterpart call via OpenAI Realtime. Without a key it falls back to a deterministic
    typed reply, Call is hidden, and the app runs fully offline. Coaching scores/tips are
    always computed locally, keeping them fast and testable. Live-call audio goes to OpenAI;
    scores do not.
...
- **Android** — practice loop (scenario pick → up to three scored turns → recap → retry), not an
  open-ended chat. Type replies, or **Call** the counterpart when the API reports `voice: true`.
  The live call is the manager in the drill (barge-in, three user segments or ~90s); Coach Heather
  recaps after hang-up using the same scores UI. The FastAPI catalog is ~10 career drills; the list scrolls. After each
  typed turn the mobile client shows the same coaching payload as web (clarity / empathy /
