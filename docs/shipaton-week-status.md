...
**This page:** what landed, what is blocked, what to merge. HardTalk Pro is on the Android client. No Play upload from the agent.
...
| D3 | RevenueCat / free-tier paywall | **Android client gate** | Raise stays free. Other drills need `hardtalk_pro`. Keys stay in `local.properties`, not git. Empty-key debug still **shows** the paywall. |
...
| --- | --- | --- |
| **RevenueCat public Google SDK key** + Play IAP product | Real sandbox/store purchases. | Android gates paid drills. Empty debug key still shows the paywall. **Do not commit keys.** |
...
**[#8 D5](https://github.com/Ashishsingh009/HardTalkAI/pull/8)** — Privacy stub (HTML + FastAPI `/privacy`), Play listing copy, Console checklist, in-app Privacy links. Explicitly **not** Console access, **not** store upload. HardTalk Pro landed on this listing branch.
...
Shipaton judging can use the local Android demo. Show the HardTalk Pro sheet, then the free raise recap. Store upload is still out of scope until Console exists.
